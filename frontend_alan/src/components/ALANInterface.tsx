...
export function ALANInterface() {
    ...
    const [localScreenTrack, setLocalScreenTrack] = useState<Track | undefined>(undefined);
    ...
    useEffect(() => {
        if (!room) return;
        // ... (existing data handler code omitted for brevity if unchanged, but need to be careful with replace)
        // Wait, I should not delete the data handler.
        // Let's use specific chunks.
    ...
    useEffect(() => {
        if (localParticipant) {
            const updateTrack = () => {
                ...
                setLocalScreenTrack(screenPub?.track);

                if (camPub?.track && !localVideoTrack) addLog("VISUAL SENSORS ACTIVE");
                if (screenPub?.track && !localScreenTrack) addLog("SCREEN INTERFACE LINKED");
            }
    ...
    }, [localParticipant, localVideoTrack, localScreenTrack]);
            ...
            <div className="w-full aspect-video bg-black/60 border border-cyan-500/20 rounded-xl overflow-hidden relative shadow-[0_0_20px_rgba(0,255,255,0.1)] pointer-events-auto">
                <VideoTrack
                    trackRef={{
                        ...
                        source: localScreenTrack ? Track.Source.ScreenShare : Track.Source.Camera,
                        publication: localParticipant.getTrackPublication(localScreenTrack ? Track.Source.ScreenShare : Track.Source.Camera)!
                    }}
                    className="w-full h-full object-cover opacity-80"
                />
                <div className="absolute top-2 left-2 text-[10px] text-cyan-400 bg-black/50 px-2 rounded">
                    {localScreenTrack ? "SCR_01" : "CAM_01"}
                </div>
                {/* Reticle */}
